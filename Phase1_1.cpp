...
int main() {
    ...
    const float blockSpeed = 500.f;
    const float minSpawnTime = 2.0f;
    const float maxSpawnTime = 3.0f;
    const float maxAngle = 30.f;

    sf::RenderWindow window(sf::VideoMode(windowWidth, windowHeight), "Rat on Crack");
    window.setFramerateLimit(60);

    sf::Texture backgroundTexture;
    if (!backgroundTexture.loadFromFile("abc.jpg")) {
        std::cerr << "Error loading background image 'abc.png'" << std::endl;
