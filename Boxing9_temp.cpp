...
static constexpr uintptr_t PLAYER_ID = 0;
...
public:
    ...
    void BeginContact(b2Contact* contact) override {
        ...
        if (userDataA == FOOT_SENSOR_ID && (userDataB >= PLATFORM_ID_BASE || userDataB == GROUND_ID))
            footContacts++;
        if (userDataB == FOOT_SENSOR_ID && (userDataA >= PLATFORM_ID_BASE || userDataA == GROUND_ID))
            footContacts++;

        if ((userDataA == PLAYER_ID && userDataB == GROUND_ID) || (userDataB == PLAYER_ID && userDataA == GROUND_ID)) {
            touchedGround = true;
        }
        ...
        if (userDataA == PLAYER_ID) {
            collectibleUserData = userDataB;
            collectibleBody = fixtureB->GetBody();
        } else if (userDataB == PLAYER_ID) {
            collectibleUserData = userDataA;
            collectibleBody = fixtureA->GetBody();
        }

        if (collectibleBody) {
            if (collectibleUserData == MAGENTA_COLLECTIBLE_ID || collectibleUserData == ORANGE_COLLECTIBLE_ID || collectibleUserData == GREEN_COLLECTIBLE_ID || collectibleUserData == RED_COLLECTIBLE_ID || collectibleUserData == WHITE_COLLECTIBLE_ID || collectibleUserData == MINUS_SCORE_COLLECTIBLE_ID) {
    ...
    void PreSolve(b2Contact* contact, const b2Manifold* oldManifold) override {
        ...
        if ((userDataA >= PLATFORM_ID_BASE && userDataB == PLAYER_ID) || (userDataB >= PLATFORM_ID_BASE && userDataA == PLAYER_ID)) {
    ...
    void EndContact(b2Contact* contact) override {
        ...
        if (userDataA == FOOT_SENSOR_ID && (userDataB >= PLATFORM_ID_BASE || userDataB == GROUND_ID) && footContacts > 0)
            footContacts--;
        if (userDataB == FOOT_SENSOR_ID && (userDataA >= PLATFORM_ID_BASE || userDataA == GROUND_ID) && footContacts > 0)
            footContacts--;
    }
...
// SnapRayCastCallback is no longer needed for 'S' key functionality
/*
...
public:
    ...
    float ReportFixture(b2Fixture* fixture, const b2Vec2& point, const b2Vec2& normal, float fraction) override {
        ...
        if (userData == PLAYER_ID || userData == FOOT_SENSOR_ID) {
            return -1.0f;
        }
...
int main() {
    ...
    const float playerJumpForce = 400.0f;
    ...
    const float fastFallGravityScale = 100.0f; // Increased gravity scale for fast fall
    ...
    if (!collectibleTextures[0].loadFromFile("CHEEZE.png")) {
        std::cerr << "Error loading texture 'CHEEZE.png'" << std::endl;
        return 1;
    }
    if (!collectibleTextures[1].loadFromFile("Pizza2.png")) {
        std::cerr << "Error loading texture 'Pizza2.png'" << std::endl;
        return 1;
    }
    if (!collectibleTextures[2].loadFromFile("Long_Platform_Green.png")) {
        std::cerr << "Error loading texture 'Long_Platform_Green.png'" << std::endl;
        return 1;
    }
    if (!collectibleTextures[3].loadFromFile("Short_Platform_Red.png")) {
        std::cerr << "Error loading texture 'Short_Platform_Red.png'" << std::endl;
        return 1;
    }
    if (!collectibleTextures[4].loadFromFile("Cheese_Rain.png")) {
        std::cerr << "Error loading texture 'Cheese_Rain.png'" << std::endl;
        return 1;
    }
    if (!collectibleTextures[5].loadFromFile("Poison.png")) {
        std::cerr << "Error loading texture 'Poison.png'" << std::endl;
        return 1;
    }
    ...
    playerFixtureDef.userData.pointer = PLAYER_ID;
    ...
    bool jumpKeyPressed = false; // This variable is not used in the provided code snippet
    bool fastFallActive = false; // Flag to indicate if fast fall is active
    ...
    while (window.isOpen()) {
        ...
        if (currentState == GameOver) {
            ...
            dt = std::min(dt, 0.1f); // Cap delta time to prevent physics glitches
            gameTime += dt;

            // Apply fast fall gravity scale if active and not grounded
            if (fastFallActive && !contactListener.isGrounded()) {
        ...
        if (currentState == Playing) {
            if (blockSpeed < maxBlockSpeed) {
                ...
                speedRatio = std::max(0.0f, std::min(1.0f, speedRatio));
            ...
            // Removed the snap logic for 'S' key
            ...
            if (spawnClock.getElapsedTime().asSeconds() >= nextSpawnTime) {
                ...
                candidateBounds.left -= 50; // Add some padding for overlap check
                ...
                if (!visualOverlap) {
                    ...
                    if (collectibleRoll < collectibleSpawnChance) {
                        ...
                        collectiblePos.y -= (fixedHeight / 2.f + collectibleRadius + 5.f); // Spawn slightly above the platform
                        ...
                        b2BodyDef collectibleBodyDef;
                        collectibleBodyDef.type = b2_kinematicBody; // Collectibles move with platforms
                        ...
                        // Set collectible velocity to match the platform velocity
                        newCollectible.body->SetLinearVelocity(newBlock.body->GetLinearVelocity());

                        collectibles.push_back(newCollectible);
                    }
                } else {
                    // If block overlaps visually, destroy its body immediately
            ...
            for (auto& block : blocks) {
                if (block.body) {
                    ...
                    // Mark blocks for removal if they move off-screen
            ...
            for (auto& collectible : collectibles) {
                if (collectible.body) {
                    ...
                    // Mark collectibles for removal if they move off-screen or fall off
            ...
            // Process collectibles marked for removal by contact listener
            for (b2Body* bodyToRemove : collectiblesToRemoveFromWorld) {
                for (auto it = collectibles.begin(); it != collectibles.end(); ++it) {
                    if (it->body == bodyToRemove) {
                        ...
                        it->markedForRemoval = true; // Mark for removal from vector
                        break; // Found the collectible, break inner loop
                    }
                }
            }
            collectiblesToRemoveFromWorld.clear(); // Clear the list for the next step

            // Remove marked blocks and destroy their Box2D bodies
            blocks.erase(std::remove_if(blocks.begin(), blocks.end(), [&](Block& block) {
                if (block.markedForRemoval && block.body) {
                    ...
                    block.body = nullptr; // Important: set to nullptr after destroying
                    return true;
                }
                return false;
            }), blocks.end());

            // Remove marked collectibles and destroy their Box2D bodies
            collectibles.erase(std::remove_if(collectibles.begin(), collectibles.end(), [&](Collectible& collectible) {
                if (collectible.markedForRemoval && collectible.body) {
                    ...
                    collectible.body = nullptr; // Important: set to nullptr after destroying
                    return true;
                }
                return false;
            }), collectibles.end());
            ...
            if (contactListener.touchedGround || playerBody->GetPosition().y > toMeters(sf::Vector2f(0, windowHeight + playerHeight)).y || playerBody->GetPosition().x < toMeters(sf::Vector2f(-playerWidth, 0)).x) {
